...
date: "Created October 22, 2019; Updated November 10, 2019"
...
The heterogeneity quesiton asks whether a relationship between two random variables has changed across conditions. It is often fundamental to a scientific inquiry. For example, a biologist could ask whether the relationship between two genes has been modified in a cancer cell from that in a normal cell. The 'DiffXTables' R package answers the heterogeneity question via evaluating statistical evidence for distributional changes in the involved variables from the observed data.
...
<!--Given two or more contingency tables as parameter x, `cp.chisq.test()` will provide two quantities to evaluate if the tables has any kind of differentialilty (first, second or both). It includes the comparative chi-square test statistic and statistical significance ($p$-value)-->
...
<!--
Given two or more contingency tables as parameter x, `heterogenity.test()`  will provide two quantities to evaluate if the tables are first order differential or second order differential but not always. It includes the heterogeinty test statistic and statistical significance ($p$-value)
-->
...
<!--
From the example it can be observed that only `cp.chisq.test()` and `heterogenity.test()` can determine this as First order differential whereas only`sharma.song.test()`  identifies the tables as not differential.
-->
...
At $\alpha=0.05$, `cp.chisq.test()` correctly declares differential patterns; `sharma.song.test()` correctly delcares second-order differential patterns; and `heterogenity.test()` correctly declares differential patterns.
...
<!-- From the example it can be observed that only `cp.chisq.test()` and `sharma.song.test()` can determine this as second order differential whereas `heterogenity.test()` identify the tables as not differential.-->
```{r, out.width='60%',fig.asp=0.6}
tables <- list(
  matrix(c(
    50,  0, 0,  0,
